...
def process_file1(file_name: str) -> pd.DataFrame:
    ...
    # Split match_id into components
    split_cols = ['match_date', 'tour', 'tournament_name', 'round', 'p1_name', 'p2_name']
    df[split_cols] = df['match_id'].str.split('-', expand=True)
    ...
    df['tournament_name'] = df['tournament_name'].str.replace('_', ' ')
    df['p1_name'] = df['p1_name'].str.replace('_', ' ')
    df['p2_name'] = df['p2_name'].str.replace('_', ' ')
