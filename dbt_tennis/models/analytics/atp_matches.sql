...
atp_odds_m as (
  select * from {{ ref('atp_odds_m') }}
),

atp_odds_p as (
  select * from {{ ref('atp_odds_p') }}
),
...
final as (
    ...
    from matches_atp as m
      ...
      left join atp_stats as s on (m.player_1_id = s.player_1_id and m.player_2_id = s.player_2_id 
                                  and m.tournament_id = s.tournament_id and m.round_id = s.round_id)
      left join atp_odds_m as om on (m.player_1_id = om.player_1_id and m.player_2_id = om.player_2_id 
                                  and m.tournament_id = om.tournament_id and m.round_id = om.round_id)
      left join atp_odds_p as op on (m.player_1_id = op.player_1_id and m.player_2_id = op.player_2_id 
