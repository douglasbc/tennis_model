...
wta_last_52_serve as (
  select * from {{ source('raw_layer', 'wta_last_52_serve') }}
),

wta_last_52_return as (
  select * from {{ source('raw_layer', 'wta_last_52_return') }}
),

wta_last_52_rally as (
  select * from {{ source('raw_layer', 'wta_last_52_rally') }}
),

wta_last_52_tactics as (
  select * from {{ source('raw_layer', 'wta_last_52_tactics') }}
),
...
final as (
    ...
    from wta_last_52_serve as s
      left join wta_last_52_return as ret
        on s.player_name = ret.player_name
      left join wta_last_52_rally as rally
        on s.player_name = rally.player_name
      left join wta_last_52_tactics as t
