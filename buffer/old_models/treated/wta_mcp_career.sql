...
wta_career_serve as (
  select * from {{ source('raw_layer', 'wta_career_serve') }}
),

wta_career_return as (
  select * from {{ source('raw_layer', 'wta_career_return') }}
),

wta_career_rally as (
  select * from {{ source('raw_layer', 'wta_career_rally') }}
),

wta_career_tactics as (
  select * from {{ source('raw_layer', 'wta_career_tactics') }}
),
...
final as (
    ...
    from wta_career_serve as s
      left join wta_career_return as ret
        on s.player_name = ret.player_name
      left join wta_career_rally as rally
        on s.player_name = rally.player_name
      left join wta_career_tactics as t
