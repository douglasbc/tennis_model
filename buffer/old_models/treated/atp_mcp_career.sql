...
atp_career_serve as (
  select * from {{ source('raw_layer', 'atp_career_serve') }}
),

atp_career_return as (
  select * from {{ source('raw_layer', 'atp_career_return') }}
),

atp_career_rally as (
  select * from {{ source('raw_layer', 'atp_career_rally') }}
),

atp_career_tactics as (
  select * from {{ source('raw_layer', 'atp_career_tactics') }}
),
...
final as (
    ...
    from atp_career_serve as s
      left join atp_career_return as ret
        on s.player_name = ret.player_name
      left join atp_career_rally as rally
        on s.player_name = rally.player_name
      left join atp_career_tactics as t
